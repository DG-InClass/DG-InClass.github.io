...
description: Instructor guide for browser events, event listeners, event objects, delegation, bubbling, and capturing
draft: true
---
...
<Instructor minutes="125-155">
- Lecture: **30-40 minutes**
- Demo Walkthrough: **95-115 minutes**

> ⚠️ Be mindful of delivery time; this walkthrough can easily run long if discussion expands.

- This is one of the longest and densest walkthroughs in the current sequence.
- Students type through project setup, page navigation, a main coordinator, click, mouse, and keyboard events, panorama panels, delegation, and bubbling/capturing.
- The walkthrough needs time for browser interaction, event-object inspection, and comparing direct listeners with delegated listeners.
- The estimate allows extra time for slower typing, event-debugging, and discussion around event flow.
</Instructor>
...
Students have already used JavaScript to select and change DOM elements. Event listeners add the next browser idea: instead of running all changes immediately, JavaScript can wait for a user action and respond when that action happens.
...
<Aside type="note" title="Old Starter Fit">
...
This Lesson demo is adapted from an older event-listener starter kit. The project is already initialized, so students do not need to run `pnpm init` or install packages one by one.
...
1. Install the prepared dependencies.
...
1. Add a click listener that toggles a class on the body.
...
1. Add a second click listener that changes message text.
...
1. Add `mouseover` and `mouseout` listeners.
...
1. This listener responds to the `'keydown'` event. That means *every* keypress results in this listener being fired/executed. Notice what happens when you press <kbd>shift</kbd> or <kbd>caps lock</kbd>. 
...
1. There are lots of other <a href="https://developer.mozilla.org/en-US/docs/Web/API/KeyboardEvent#events" target="_blank">keyboard events</a>. What do you think are the purposes of these events?
...
The panorama sample lives on `panorama.html`. It combines mouse events with event objects, event delegation, class changes, and conditional UI text. It uses one full panorama image and creates the slice effect with CSS background positioning, so students do not need to do any image processing during the lesson.
...
1. Add one `mouseover` listener to the parent container.
...
1. Add a `mouseout` listener that checks `event.relatedTarget`.
...
1. Add one listener to the list.
...
1. Notice the use of `event.target.closest('li')` to find the list item. But what was the target that was actually clicked on? Consider adding a `console.log(event.target);` to inspect it in the dev tools' console.
...
1. One final experiment: Let's remove the search for the `list` and `selection` elements and see what happens.
    ...
    <Aside type="tip" title="Unique IDs on Elements">
...
1. Create a helper that writes to the event log.
...
1. Add bubbling listeners. What happens when you click on each area?
...
1. Add a clear button. This can "reset" the display area so you can explore the effects of your clicking more carefully.
...
1. Add extra listeners on the `outerBox` and `middleBox`, this time with a slightly different message about "capturing" the click. Make special note about the *order* of the messages that appear in the `eventLog` element.
...
1. What happens if you add a *third* parameter to the `addEventListener()`? Make the following changes by using `true` for the third argument - `useCapture`.
...
Students should leave this lesson with a practical event workflow:
...
<Aside type="tip" title="Event Listener Habit">
  Ask students to say which element is listening and which element triggered the event. That distinction is the key to understanding delegation, bubbling, and capturing.
</Aside>
