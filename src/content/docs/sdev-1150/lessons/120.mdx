---
title: Repetition - Flow Control Intro
description: Instructor guide for while loops, for loops, accumulators, break, and nested loops
draft: true
---
...
- This lesson is longer than lesson 02 and introduces a new flow-control pattern.
- Students type through loop selection, starter script setup, saving calculations, study-session totals, stop conditions, nested loops, and optimization.
- The walkthrough needs time for commentary around repeated execution, loop conditions, and how nested loops change the mental model.
- The estimate allows extra time for slower typing and for students to trace changing values across loop iterations.
</Instructor>
...
Students should have completed homework that introduced repetition using looping statements. This lesson reinforces that concept with new examples. Do not repeat the homework samples. The goal is to help students recognize the same loop patterns in different problem settings.

### Flow Control

Repetition is the third major flow-control idea students have now seen:
...
<Aside type="note" title="Loop Anatomy">
  A useful way to read loops is to look for four pieces:

  - the starting state
  - the condition
  - the repeated work
  - the update that moves the loop toward stopping
</Aside>
...
<Aside type="caution" title="Infinite Loops">
  A loop that never reaches its stopping condition can keep running forever.

  When students write a loop, ask them to identify which value changes and how that change helps the loop stop.
</Aside>
...
We're going back to a stand-alone JavaScript file for this lesson. There is no HTML, no CSS, no package setup, and no modules. Run the file directly with Node so students can focus on the loop behaviour.
...
1. Add the `while` loop.
    ...
    <Aside type="note" title="What Changes?">
...
1. Add conditional logic and accumulators.
...
1. Calculate and display the average.
    ...
    <Aside type="note" title="Accumulator Variables">
...
1. Loop until the running total reaches the target.
...
1. Display the result.
    ...
    <Aside type="caution" title="Use Break Intentionally">
...
1. Add the inner loop.
    ...
    <Aside type="note" title="Nested Loop Reading Habit">
...
> **Assigned homework:** Complete [Homework: Practice Loop Control](./homework/130-homework.mdx). It reinforces today's repetition work and gives fallback practice for `while` loops, stop conditions, nested loops, and improving code after it works.
...
<Aside type="tip" title="Understanding JavaScript 👀">
  ...
  In every loop, ask: What is the starting state? What condition is checked? What work repeats? What update moves the loop toward stopping?
</Aside>
