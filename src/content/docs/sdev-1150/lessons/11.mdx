...
<When planned='Feb 12' actual='Feb 23' />


import preview from './previews/lesson-11.png';
...
<Aside title="Summary">
It's not just about if/else checks. It's about user feedback and leveraging built-in HTML validation attributes alongside custom validation.

Also, given that we are doing client-side processing (no server-side to receive form submissions), we will always use `.preventDefault()` on the `'submit'` event of forms.
</Aside>
