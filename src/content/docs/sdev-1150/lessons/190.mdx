...
description: Instructor guide for asynchronous browser requests with fetch, JSON Server, and DOM rendering
draft: true
---
...
1. Send the single-body request.
...
1. Confirm both responses return JSON.
</Steps>

<Aside type="tip" title="Checkpoint">
Students should know the API is healthy before debugging any `fetch()` code.
...
1. Add a shared response handler.
...
1. Add a POST helper using `async` and `await`.
...
1. Select the DOM elements and define the endpoint.
...
1. Create small rendering helpers for status text and the body list.
...
<Aside type="tip" title="Checkpoint">
Students should now be able to load the seed bodies and add a new body from the browser.
...
> **Assigned homework:** Complete [Homework: Practice Fetch Helpers](./homework/200-homework.mdx). It reinforces today's Fetch API workflow and gives fallback practice for helper functions, JSON parsing, rendering, loading states, and async/API errors.
