---
title: Consuming REST API Endpoints
description: Instructor guide for guarded API calls, related REST resources, non-JSON fetches, and client-side data synthesis
draft: true
---
...
- Lecture: **35-45 minutes**
- Demo Walkthrough: **75-95 minutes**
...
- Students type through guarded fetch helpers, static backend files, joined catalog data, detail rendering, and add/edit/delete review workflows.
- The walkthrough needs time for response guarding, data-shape reasoning, rendering state, and REST endpoint troubleshooting.
- The estimate allows extra time for slower typing, server/browser coordination, and debugging multi-step API interactions.
</Instructor>
...
- `reviews`: user-generated data that students can add, edit, and delete.

The app does not mutate `books` or `authors`. Those resources behave like reference tables. The only mutable resource is `reviews`, so the UI gives students a clear boundary between reading stable data and changing user-owned data.

### Guarding Responses

The Fetch API resolves its promise when the server sends a response, even if that response is a `404` or `500`. Students need to check `response.ok` before reading the body:
...
Students should also inspect the response header before choosing how to read the body:
...
<Aside type="note" title="Lesson Boundary">
  Keep this lesson focused on consuming and synthesizing local endpoints. Authentication, pagination, and external API keys are outside the scope of this demo.
</Aside>
...
1. Confirm the terminal shows both `frontend` and `api-server` output.
...
1. Send the request for all books.
...
1. Send the request for all authors.
...
1. Send the request for all reviews.
...
1. Send one markdown request and one jacket image request.
...
Students should know the REST resources and static files are healthy before debugging frontend code.
...
1. Stop `pnpm dev` with <kbd>Ctrl</kbd> + <kbd>C</kbd>.
...
1. Keep the smoke-test URLs pointed directly at JSON Server, such as `http://localhost:3000/books`.

1. In browser JavaScript, use same-origin paths such as `/books` and `/jackets/orbiting-javascript.svg`. Vite will forward those requests to JSON Server during development.
</Steps>

<Aside type="tip" title="Checkpoint">
Students should be able to explain that REST Client is not blocked by browser CORS, but browser JavaScript is. The Vite proxy keeps browser requests same-origin while still using JSON Server as the backend.
...
1. Check HTTP status with `response.ok`.
...
1. Check the `content-type` header before reading the body.
...
1. Add text and blob helpers for non-JSON responses.
...
1. Add a JSON-sending helper for POST, PATCH, and DELETE requests.
...
1. Use same-origin paths so browser requests go through the Vite proxy.
    ...
    <Aside type="note" title="Why `bookId` Is Numeric">
      ...
      Since `bookId` is a normal data field on a review, this lesson stores it as a number. That keeps the filtered review endpoint simple while still giving students a useful type-awareness moment.
    </Aside>

1. Fetch books, authors, and reviews together.
...
1. Render one catalog button per book.
...
1. Start the related requests together.
...
1. Convert the jacket blob into an object URL.
...
1. Revoke the previous object URL before replacing it.
...
<Aside type="note" title="Optional Demo">
  Use this section only if time allows or if students are curious about the earlier CORS warning. The main lesson works without pausing for this demonstration.
</Aside>
...
1. Temporarily comment out the `/summaries` and `/jackets` proxy entries.
...
1. Reload the app and select a book.

1. Open DevTools and check the Console or Network tab.
...
1. Restore the `/summaries` and `/jackets` proxy entries.
...
1. Restart `pnpm dev` again.

1. Reload the app and confirm the detail view works again.
</Steps>
...
1. POST a new review from the add-review form.
...
1. PATCH an existing review from an edit form.
...
1. DELETE a review from its delete button.
...
1. Reload the catalog and selected detail after each mutation so review counts and review cards stay current.
</Steps>

<Aside type="tip" title="Checkpoint">
Students should be able to add, edit, and delete reviews without changing books or authors.
...
> **Assigned homework:** Complete [Homework: Practice Guarded API Rendering](./homework/210-homework.mdx). It reinforces today's API consumption pattern and gives fallback practice for guarded fetch helpers, joined data, detail rendering, and review request workflows.

Students should finish this lesson with a stronger API consumption pattern. They should check status and content type, choose the correct body-reading method, combine endpoint responses by id, and keep mutable user data separate from stable reference data.
