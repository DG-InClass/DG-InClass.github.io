...
description: Capture, process, and validate HTML form input with JavaScript.
draft: true
---
...
<Instructor minutes="70-95">
- This lesson is shorter than the longest browser demos but introduces two related form workflows.
- Students type through submit handling, preventing reloads, reading form input, echoing values, built-in validation, and custom validation.
- The walkthrough needs time for browser testing and discussion around when the form is allowed to process.
- The estimate allows extra time for slower typing and for troubleshooting validation states.
</Instructor>
...
This lesson combines two related form workflows. First, students capture form input and echo it back to the page without allowing the browser to reload. Second, students add validation so the same form only gets processed after it passes the built-in and custom rules.
...
1. Create a helper function that gathers the current form values.
...
1. Format the submitted data for the output area.
...
1. Listen for the `submit` event. Then serialize the form and echo the result.
...
1. Route each edited field to the correct validation helper.
...
1. Add helpers for full-form validation and cleanup.
...
1. Update the submit listener so invalid data is not processed.
...
1. Update the reset listener so custom validity messages are cleared.
...
<Aside type="tip" title="Refactoring the Code">
  Ask the students how they might have structured the code for this demo if they were to do it all on their own. Note that this is but *one* possible way to arrange the code, and it's not necessarily the *best* way.
</Aside>

> **Assigned homework:** Complete [Homework: Practice Form Validation](./homework/170-homework.mdx). It reinforces today's form submission workflow and gives fallback practice for validation checks, custom rules, and user-facing feedback.

By the end of this demo, students should have one form that supports both sides of client-side form handling. The first part shows how JavaScript can intercept a submission, read named form controls, and echo the submitted data without a page reload. The second part adds the validation layer so the same submission flow only runs after the browser and the custom JavaScript rules agree that the input is acceptable.

The important connection is that validation does not replace form processing. Validation protects the processing step. Once students see that relationship, `submit`, `preventDefault()`, `form.elements`, `setCustomValidity()`, `checkValidity()`, and `reportValidity()` fit together as one practical workflow.
