...
description: Instructor guide for selecting, navigating, creating, inserting, replacing, and removing DOM nodes
draft: true
---
...
- Demo Walkthrough: **85-105 minutes**

> ⚠️ Be mindful of delivery time; this walkthrough can easily run long if discussion expands.

- This lesson is substantially longer and more browser-focused than lesson 02.
- Students type through project setup, DOM selection, subtree searching, navigation, modifying elements, creating nodes, inserting images, replacing/removing elements, fragments, and credits.
- The walkthrough needs time for browser verification and discussion around the difference between selecting existing nodes and creating new ones.
- The estimate allows extra time for slower typing, DOM inspection, and debugging page-visible results.
</Instructor>
...
Lesson 12 introduced the first habit of DOM work: select an element, log it, confirm JavaScript found what was expected, and then change one thing about it.

This lesson goes deeper. Students should now see the DOM as a tree they can search, navigate, and modify while the page is running.
...
<Aside type="note" title="Element vs Node">
  The DOM contains different kinds of nodes. Elements are nodes, but text is also represented by text nodes.

  Use element-focused properties such as `children`, `firstElementChild`, and `nextElementSibling` when students only need element nodes.
</Aside>

### Creating DOM Nodes

Students should distinguish between changing an existing element and creating a new one:
...
The Lesson 15 demo carries forward the browser starter from Lesson 12. The project is already initialized, so students do not need to run `pnpm init` or install packages one by one.
...
1. Import PicoCSS and log a first message in `main.js`.
...
1. Convert the `NodeList` into an array when array methods are useful.
...
1. Check a selection that fails.
    ...
    The console shows `null`. Students should check for `null` before changing an element that might not exist.
</Steps>
...
1. Navigate between sibling elements.
    ...
    <Aside type="note" title="Element Properties">
...
1. Change the existing aside image attributes.
...
1. So far, all we've done with `reminder`, `reminderLabel` and `reminderText` is generate **document fragments**. They are all separate from each other. Let's assemble them.
    ...
    <Aside type="note" title=".append() or .appendChild()?">
...
1. While we have assembled the `reminder` paragraph, it's still not part of the DOM. Let's add that to the DOM.
...
1. Remove the horizontal rule after the list.
...
Students should leave this lesson with a practical DOM workflow:
...
<Aside type="tip" title="DOM API Habit">
  Keep asking students what kind of value each operation returns: one element, a collection, a text node, a new element, or nothing visible.
</Aside>
