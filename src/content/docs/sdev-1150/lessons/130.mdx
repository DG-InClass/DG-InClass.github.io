---
title: Looping with Arrays
description: Instructor guide for processing arrays with loops, callbacks, and common array methods
draft: true
---
...
<Instructor minutes="105-135">
> ⚠️ Be mindful of delivery time; this walkthrough can easily run long if discussion expands.

- This lesson is substantially longer and denser than lesson 02.
- Students type through manual loops, callback syntax, array method purposes, mapping, finding, filtering, slicing, splicing, sorting copied arrays, and optional script arguments.
- The walkthrough needs time to compare manual iteration with array methods and to discuss when each method communicates intent clearly.
- The estimate allows extra time for slower typing, callback syntax questions, and careful discussion of methods that return new arrays versus methods that mutate arrays.
</Instructor>
...
Students already know arrays as a structure: an array holds ordered values, each value has an index, and `.length` reports how many values the array contains. This lesson moves from knowing what an array is to processing each item in an array.
...
<Aside type="note" title="Manual Loop">
  A manual loop makes the indexes visible. This is useful when students need to understand how the program moves from one array position to the next.
</Aside>
...
<Aside type="tip" title="Array Method">
  An array method hides the repeated index movement and asks students to focus on the operation for each item.
</Aside>
...
We're staying in a stand-alone JavaScript file for this lesson. There is no HTML, no CSS, no package setup, no modules, and no browser API work. Run the file directly with Node so students can focus on array processing.
...
<Aside type="note" title="Optional Detour">
  If students are comfortable with the starter script, there is an [optional script arguments detour](#optional-script-arguments) at the end of this guide.
</Aside>

### Manual Loop Over Study Minutes

Start with the pattern students already saw in the repetition lesson: a `for` loop that uses an index and `.length`.
...
1. Loop through the array by index.
    ...
    <Aside type="note" title="Index vs Display Number">
      The index starts at `0`, but the display label uses `index + 1` because students usually expect the first day to be called day 1.
    </Aside>
</Steps>
...
Show `.forEach()` before `.map()` so students can separate two ideas:
...
1. Use `.map()` to pull out task titles.
...
1. Find the first incomplete task and the first high-priority task index.
    ...
    <Aside type="note" title="First Match">
...
Use `.slice()` when students need part of an array but should leave the original array alone. Use `.splice()` when students intentionally want to remove, replace, or insert items in an array.
...
1. Make a copy, then use `.splice()` on that copy.
    ...
    <Aside type="caution" title="Mutating Method">
      `.splice()` changes the array it is called on. The demo uses `editableTasks` so students can see the mutation without disturbing the original `courseTasks` data used later.
    </Aside>
</Steps>
...
1. Sort by a number field and a string field.
    ...
    <Aside type="caution" title="Compare Function">
...
Today's lesson connects the loop mechanics students already know with the array methods they will see often in JavaScript code.

Students should be able to describe the difference between these patterns:
...
<Aside type="tip" title="Choosing the Tool">
  Ask students to name the result they need before choosing a method.
...
If students are comfortable with the main demo, add this short side example after the core array-method walkthrough. It shows how command-line arguments become array data.
...
1. Run the file with Node's watch mode and pass a few values after the script filename.
    ...
    The values students typed after `demo-args.js` start at index `2`.

1. Add a small loop that prints only the student-provided arguments.
...
<Aside type="note" title="Optional Detour">
  Keep this example short. Its purpose is to show that arrays appear in real Node programs, not to turn the lesson into command-line application design.
</Aside>
