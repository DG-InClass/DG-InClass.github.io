---
title: Reactive Patterns and Advanced UI Communication
description: SUMMARY
sidebar:
  label: 03 - Custom Events
---
...
<When planned='Sep 15' actual='' />

## Lecture

> *A computer program is a **set of instructions** for manipulating **information**.*

In this lesson, we're goint to add the ability to display a resource's details in the details component when a result item is clicked.
...
<Tabs syncKey='section'>
  <TabItem label='A01'>
    ```ps
    pnpm dlx tiged --disable-cache --force DG-InClass/SDEV-2150-A01-Sep-2026/sk/day-03/example/lesson-03-starter ./src/lesson-03
    ```
  </TabItem>
  <TabItem label='A02'>
    ```ps
    pnpm dlx tiged --disable-cache --force DG-InClass/SDEV-2150-A02-Sep-2026/sk/day-03/example/lesson-03-starter ./src/lesson-03
    ```
  </TabItem>
</Tabs>
...
<Steps>
1. Add the following to the `main.js`.
...
1. Modify the template template string in `resource-results.js` to remove all the hard-coded buttons that contain the resource results.

    ```diff lang="jsx" title="src/js/components/resource-results.js"
...
1. Then, modify the component class to dynamically accept the data.
...
1. Update the `render()` function to build the component's content from the supplied data. This is where we dynamically build each `<button>` that we removed from the template string.
...
1. Create an event handler for button clicks in the resource results.
...
1. Make sure the event handler is hooked up in both the constructor and the connected callback.
...
1. Also make sure to detach the event handler in the disconnnected callback.
...
We will need to do similar work with the `resource-details.js` custom element. This time, however, we'll replace the hard-coded content with a `<slot>` where the details can be injected.

<Steps>
1. First, modify the template to remove hard-coded information.
...
1. Next, we modify the component class to process the data dynamically. We'll add a `#resource` private property to hold the data and a *setter* so its value can be set at runtime.
...
1. Lastly, we'll adjust the rendering to build up the content. This time, we'll do it the "better" way by using `document.createElement()` instead of trying to build HTML from a template string.
...
You should also try to stretch what you've reviewed so far by adding these features to today's demo:
