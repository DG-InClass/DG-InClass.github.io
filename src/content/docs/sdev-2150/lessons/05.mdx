...
description: SUMMARY
sidebar:
  label: 05 - Asynchronous Programming
---
...
<Aside type="note" title="REST-ful Fetching">
  It helps to recall how [HTTP and Server-Side Endpoints](/sdev-1150/lessons/180/) work along with [`fetch()` calls](/sdev-1150/lessons/190/).
</Aside>
...
<Tabs syncKey='section'>
  <TabItem label='A01'>
    ```ps
    pnpm dlx tiged --disable-cache --force DG-InClass/SDEV-2150-A01-Sep-2026/sk/day-05/example/lesson-05-starter ./src/lesson-05
    ```
  </TabItem>
  <TabItem label='A02'>
    ```ps
    pnpm dlx tiged --disable-cache --force DG-InClass/SDEV-2150-A02-Sep-2026/sk/day-05/example/lesson-05-starter ./src/lesson-05
    ```
  </TabItem>
</Tabs>

The first thing to note about this starter kit is that it's comprised of two projects:

<FileTree>
- backend/ Data provided via a separate project
- frontend/ Web project consuming a backend API
</FileTree>

The `backend/` project is a simple server running on port 3000 that exposes the following endpoints:
...
To understand the structure of the data being represented, examine the `backend/resources.json` and the `backend/README.md`.

### Running the Projects

Given that we are working with two projects, both need to be started in the terminal.
